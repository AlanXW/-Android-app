...
public class LoginActivity extends BaseActivity implements OnClickListener {
    ...
    private void login() {
        ...
        if (TextUtils.isEmpty(username) || TextUtils.isEmpty(password)) {
            Toast.makeText(mContext, "Can not be blank", Toast.LENGTH_SHORT).show();
            return;
        }
        checkUser();
...
private void echo() {
    Map<String, String> map = SharedPreferencesUtils.getUserInfo(mContext);
    if (map != null) {
        ...
        public void onResponse(String response, int id) {
            ...
            switch (id) {
                case 1:
                    ...
                    if (user.getUserId() == 0) {
                        DisplayToast("User name or password error");
                        return;
                    } else {
                        // 存储用户
                        Constants.USER = user;
                        ...
                        if (result) {
                            ...
                            Toast.makeText(mContext, "User storage failure", Toast.LENGTH_SHORT).show();
                        }
                    }
                    openActivity(MainActivity.class);
                    finish();
